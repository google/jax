...
# based on https://github.com/openxla/xla/blob/a7a09d56c3599123f8148bbf3e44c9ebc04624b9/xla/mlir_hlo/mhlo/transforms/chlo_legalize_to_hlo/chlo_legalize_to_hlo.cc#L644-L694
...
def _erf_inv_64_helper(x):
  ...
  return jnp.where(np.abs(x) == 1.0, np.inf * x, p * x)
...
def _erf_inv_lowering_rule(ctx: LoweringRuleContext, x):
  ...
  if x_aval.dtype == jnp.float64:
    return lower_fun(_erf_inv_64_helper, multiple_results=False)(ctx, x)
