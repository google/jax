...
def _handle_indexing(
  ...
  if any(isinstance(t, gpu_core.ReshapeRef) for t in transforms[:-1]):
    if not all(isinstance(t, gpu_core.ReshapeRef) for t in transforms[:-1]):
    ...
    # be.
    ref = mgpu.memref_reshape(ref, transforms[-2].shape)
