...
def fresnel(x: ArrayLike) -> tuple[Array, Array]:
  ...
  Notes:
     ...
     `here
     <https://github.com/scipy/scipy/blob/1abc9faf43d7b82e0b26589bb705d8d009b18713/scipy/special/special/cephes/fresnl.h>`_.
  """
...
def _fresnel_jvp(primals, tangents):
  x, = primals
  x2 = x * x
  x2_scaled = x2 * jnp.pi / 2
  x_dot, = tangents
  result = fresnel(x)
  dSdx = jnp.sin(x2_scaled) * x_dot
  dCdx = jnp.cos(x2_scaled) * x_dot
  return result, (dSdx.astype(result[0].dtype), dCdx.astype(result[0].dtype))
