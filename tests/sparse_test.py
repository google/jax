...
class SparseGradTest(sptu.SparseTestCase):
  ...
  def test_sparse_pytree_grad(self, has_aux, deep, arg0, bias):
    ...
    with self.subTest("wrt sparse"):
      ...
      # Pytree structure
      def map_todense(x):
        if isinstance(x, sparse.bcoo.BCOO):
          return x.todense()
        else:
          return x
      bcoo_to_leaf = lambda x: isinstance(x, sparse.bcoo.BCOO)
      grad_densified = jax.tree_util.tree_map(map_todense, grad_sp,
                                              is_leaf=bcoo_to_leaf)
