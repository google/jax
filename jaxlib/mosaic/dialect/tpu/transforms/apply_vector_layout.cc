...
LogicalResult tpu_matmul_rule(RewriteContext &ctx, Operation &op,
  ...
  int64_t nj;
  // TODO(mvoz): Legacy RHS transpose path.
  if (transpose_rhs) {
  ...
  // Note - at this point we've handled transposition (above and earlier passes)
  // and batching (earlier passes) - so this is a vanilla mkn matmul.
  ...
  for (int64_t j = 0; j < nj; ++j) {
    for (int64_t k = 0; k < nk; ++k) {
      ...
      auto new_acc_col = builder.create<tpu::MatmulOp>(
          ...
          // Note: transpose_rhs is legacy here and will be removed soon.
          // The only way to explicitly still have a transpose_rhs is to
          // set the transpose_rhs attribute on the op, and not set
          // dimension_numbers.
          false, transpose_rhs, precision_attr, dot_dimension_numbers_attr);
...
LogicalResult tpu_concatenate_rule(RewriteContext &ctx, Operation &op,
  ...
  auto minor_most_dims = res_ty.getRank() - res_layout->layout_rank();
  ...
  for (int i = 0; i < op.getNumOperands(); ++i) {
    ...
    operand.setType(VectorType::get(
        operand_shape, cast<VectorType>(operand.getType()).getElementType()));
    FAILUREOR_ASSIGN_OR_RETURN(
  ...
  if (!tiling_dim.has_value()) {
    ...
    if (res_layout->offsets() != LayoutOffsets{0, 0}) {
